...
import { useNotesForProjectUpdate, useCreateNote, useUpdateNote, useDeleteNote } from '@/hooks/use-notes';
...
interface UpdateNotesThreadProps {
...
export function UpdateNotesThread({
  projectUpdateId,
  projectId,
  dateOfReview,
  author,
}: UpdateNotesThreadProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [isAddingNote, setIsAddingNote] = useState(false);
  const [editingNote, setEditingNote] = useState<Note | null>(null);

  // Fetch notes for this specific project update
  const { data: notes, isLoading, error } = useNotesForProjectUpdate(projectUpdateId);
