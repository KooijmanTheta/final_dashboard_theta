...
import { VehicleUpdateNotesThread } from './vehicle-update-notes-thread';
import { cn } from '@/lib/utils';
import { Calendar, FileText, ChevronDown, ChevronUp } from 'lucide-react';

interface VehicleUpdateCardProps {
...
const TAG_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  'Comparable Mention': { bg: 'bg-slate-100', text: 'text-slate-700' },
};
...
export function VehicleUpdateCard({
  update,
  vehicleId,
  dateOfReview,
  author,
  ...
  // Combine note_tags and source_document_type for display
  const displayTags: string[] = [];
  if (update.source_document_type) {
    displayTags.push(update.source_document_type);
  }
  if (update.note_tags) {
    displayTags.push(...update.note_tags.filter(tag => tag !== update.source_document_type));
  }
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {update.entity_type && update.entity_type !== 'Vehicle' && (
        ...
        {/* Notes Thread */}
        <VehicleUpdateNotesThread
          vehicleUpdateId={update.id}
          vehicleId={vehicleId}
